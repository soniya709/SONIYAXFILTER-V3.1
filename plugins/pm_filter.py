...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    # don't change anything without contacting me @creatorrio
        ...
        buttons = [[
            InlineKeyboardButton('📲 ꜱᴇɴᴅ ᴘᴀʏᴍᴇɴᴛ ꜱᴄʀᴇᴇɴꜱʜᴏᴛ ʜᴇʀᴇ', url=f"https://t.me/{PAYMENT_ADMIN_USERNAME}")
        ],[
        ...
        btn = [[
            InlineKeyboardButton('📲 ꜱᴇɴᴅ ᴘᴀʏᴍᴇɴᴛ ꜱᴄʀᴇᴇɴꜱʜᴏᴛ', url=f"https://t.me/{PAYMENT_ADMIN_USERNAME}")
        ],[
    ...
    # Thanks to @CoderluffyTG for fixing this 
        ...
        buttons = [[
            InlineKeyboardButton('☎️ ᴄᴏɴᴛᴀᴄᴛ ᴏᴡɴᴇʀ ᴛᴏ ᴋɴᴏᴡ ᴍᴏʀᴇ', url=f"https://t.me/{PAYMENT_ADMIN_USERNAME}")
        ],[
        ...
        buttons = [[
            ...
            InlineKeyboardButton('• ᴏᴡɴᴇʀ •', url=f"https://t.me/{PAYMENT_ADMIN_USERNAME}"),
...
async def advantage_spell_chok(client, name, msg, reply_msg, advance_search):
    mv_id = msg.id
    mv_rqst = name
    reqstr1 = msg.from_user.id if msg.from_user else 0
    reqstr = await client.get_users(reqstr1)
    ...
    movielist += [f"{movie.get('title')} {movie.get('year')}" for movie in movies]
    SPELL_CHECK[mv_id] = movielist
    if AI_SPELL_CHECK == True and advance_search == True:
        advance_search_new = False
        advance_ai_msg = await reply_msg.edit_text("<b><i>Advance Ai Try To Find Your Movie With Your Wrong Spelling.</i></b>")
        movienamelist = []
        movienamelist += [movie.get('title') for movie in movies]
        for found in movienamelist:
            try:
                mv_rqst = mv_rqst.capitalize()
            except:
                pass
            if mv_rqst.startswith(found[0]):
                await auto_filter(client, found, msg, reply_msg, advance_search_new)
                break
        reqst_gle = mv_rqst.replace(" ", "+")
        button = [[
            InlineKeyboardButton("Gᴏᴏɢʟᴇ", url=f"https://www.google.com/search?q={reqst_gle}")
